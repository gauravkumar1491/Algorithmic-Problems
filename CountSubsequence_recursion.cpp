...
int countSubseqUtil(string text, string patt, int textInd, int pattInd)
{
	...
	{
		count++;
		return countSubseqUtil(text,patt,textInd+1,pattInd);
	}
	...
	int i=textInd;
		...
		int n = patt.length();
		if(text[i]==patt[j])
			countSubseqUtil(text,patt,i+1,(j+1)%n) + countSubseqUtil(text,patt,i+1,j);
		else
			countSubseqUtil(text,patt,i+1,j);
...
{
	string patt = "QAQ";
	string text = "QAQQQZZYNOIWIN";
	cout<<countSubseq(text,patt);
	return 0;
}
